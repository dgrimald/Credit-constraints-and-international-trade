---
title: "Extracting Working data from data lake"
...
SELECT year, sector_id, mca_id, SUM(employment_wage_year*v2) AS s_m_yearly_wage, SUM(employment_wage_dec*v2) AS s_m_dec_wage
...
GROUP BY year, sector_id, mca_id
```

```{sql connection=data_lake, output.var="emp.data"}
SELECT year, sector_id, mca_id, SUM(total_emp*v2) AS s_m_emp, COUNT(DISTINCT company_record_id)*v2 as s_m_companies
...
emp.data %<>%
  filter(!is.na(sector_id)) %>% 
  full_join(wage.data, by=c("year", "sector_id", "mca_id")) %>% 
...
save(emp.data, file="../02 - Data sets/emp_data.Rdata")
rm(wage.data)
gc(reset=TRUE)
```
...
save(trade.data, file="../02 - Data sets/trade_data.RData")
...
SELECT year, mca_id, bank_id, COUNT(DISTINCT branch_id) AS n_branches, SUM(housing_loans + rural_loans + other_loans) AS total_loans, SUM(other_loans) AS other_loans, SUM(short_term_deposits+long_term_deposits+interbank_deposits) AS deposits
FROM LOCALITIES NATURAL JOIN BANK_BRANCHES
WHERE year IN (1995, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018)
GROUP BY year, mca_id, bank_id
```
...
LIM.data <- bank.data %>% 
  ...
  mutate(bank_id = as.double(bank_id),
         bank_id = case_when(bank_id %in% itau_unibanco ~ 60872504,
                             TRUE ~ bank_id)) %>%
  group_by(year, mca_id, bank_id) %>%
  summarise_all(sum) %>%
  ungroup() %>% 
  group_by(year, mca_id) %>% 
  mutate(mca_loans=sum(total_loans),
         mca_other_loans=sum(other_loans),
         mca_deposits=sum(deposits),
         mca_banks=n_distinct(bank_id),
         mca_branches=sum(n_branches)) %>%
  ungroup() %>% 
  mutate(s_loans=total_loans/mca_loans,
         s_other_loans=other_loans/mca_other_loans,
         s_deposits=deposits/mca_deposits) %>%
  group_by(mca_id, .drop=TRUE) %>% 
  summarise(n_banks = first(mca_banks),
            total_loans = first(mca_loans),
            other_loans = first(mca_other_loans),
            deposits = first(mca_deposits),
  ...
  select(mca_id, ihh_loans, ihh_other_loans, ihh_deposits, ihhN_loans, ihhN_other_loans, ihhN_deposits) %>% 
  rename(LIM_loans = ihh_loans,
         LIM_other_loans = ihh_other_loans,
         LIM_deposits = ihh_deposits,
         LIMn_loans = ihhN_loans,
         LIMn_other_loans = ihhN_other_loans,
         LIMn_deposits = ihhN_deposits) %>% 
  as.data.frame()
...
bank.data  %<>%
  ...
  ungroup() %>% 
  group_by(year, mca_id, .drop=TRUE) %>% 
  summarise(n_banks = first(mca_banks),
            n_branches = first(mca_branches),
            total_loans = first(mca_loans),
            other_loans = first(mca_other_loans),
            deposits = first(mca_deposits),
            ihh_loans = sum(s_loans^2),
...
save(bank.data, file="../02 - Data sets/bank_data.Rdata")
...
working.data <- emp.data %>% 
  left_join(bank.data, by=c("mca_id", "year")) %>% 
  left_join(trade.data, by=c("sector_id", "year")) %>%
  left_join(LIM.data, by="mca_id") %>% 
  mutate_at(vars(!starts_with(c("ihh", "LIM"))), ~ replace_na(., 0))

save(working.data, file="../02 - Data sets/working_data.Rdata")
```
