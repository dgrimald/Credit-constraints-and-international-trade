...
mun_region.crosswalk <- regions.data %>% 
  ...
  rename(nome.mun = nome_mun,
         nome.microregion = nome_rgi,
         nome.macroregion = nome_rgint, 
         id.mun = CD_GEOCODI,
         id.microregion = cod_rgi,
         id.macroregion = cod_rgint,
         id.mca = id_AMC
        ) %>% 
  select(id.mun, id.microregion, id.macroregion, id.mca, nome.mun, nome.microregion, nome.macroregion)
```
...
table1 <- as.data.frame(mun_region.crosswalk[is.na(mun_region.crosswalk$id.mca),])
kbl(table1,
    align=c("l", "l", "l"),
...
mun_region.crosswalk %<>%
  mutate(id.mca = case_when(nome.mun=="Mojuí dos Campos" ~ 2020,
                            nome.mun=="Balneário Rincão" ~ 14413,
                            nome.mun=="Pescaria Brava" ~ 14440,
                            nome.mun=="Pinto Bandeira" ~ 15038,
                            nome.mun=="Paraíso das Águas" ~ 1103,
                            TRUE ~ id.mca))
```

With that manual imputation, all the `r length(unique(mun_region.crosswalk$id.mun))` municipalities are associated with `r length(unique(mun_region.crosswalk$id.mca))` MCAs, `r length(unique(mun_region.crosswalk$id.microregion))` microregions, and `r length(unique(mun_region.crosswalk$id.macroregion))` macroregions.
